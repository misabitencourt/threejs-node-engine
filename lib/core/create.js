...
import { createNativeWindow, currentDrawableSize } from '../platform/window.js';
...
export async function create(config) {
  ...
  const { sdlWindow, gl, canvas, pixelWidth, pixelHeight, useLogicalSize, title } =
    surface;
  ...
  function setFullscreen(on) {
    ...
    const next = currentDrawableSize(sdlWindow, useLogicalSize);
  ...
  sdlWindow.on('resize', (event) => {
    const next = currentDrawableSize(sdlWindow, useLogicalSize, event);
    applyViewport(next.width, next.height);
  });
