/**
 * HUD stacking via `zIndex` (2dimagesprite, 2dbitmap, 2dtext).
 *
 * Higher zIndex draws in front. Elements are listed back-to-front in reverse
 * array order so stacking comes from zIndex, not insertion.
 *
 *   sprite  zIndex 0  — char behind the panel
 *   bitmap  zIndex 1  — rectangle over the sprite
 *   text    zIndex 2  — label on the rectangle
 *
 * Run: npm run zindex
 */
...
await engine.create({
  ...
  window: {
    title: 'threejs-node-engine · zIndex (HUD stack)',
    width: WIN_W,
    height: WIN_H,
    resizable: false,
    background: 0x0b0f14,
  ...
  elements: [
    {
      type: '2dtext',
      text: 'Front of the shape',
      position: [RECT_X + 28, RECT_Y + 48],
      fontSize: 32,
      color: '#ffffff',
      padding: 4,
      zIndex: 2,
    ...
    {
      type: '2dtext',
      text: 'zIndex · sprite 0 · rect 1 · text 2',
      position: [24, 20],
      fontSize: 22,
      color: '#9fd3ff',
      padding: 6,
