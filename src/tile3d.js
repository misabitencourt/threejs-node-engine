/**
 * 3D tile terrain + controlled cube (same idea as heightmap example).
 *
 ...
 *
 * Element: type `3dtile` · helpers in lib/utils/tile3d.js
 *
 * Run: npm run tile3d
 */
...
await engine.create({
  ...
  window: {
    title: 'threejs-node-engine · 3dtile + cube',
    width: 960,
    height: 600,
    resizable: false,
    background: 0x7ec8e3,
    ...
    gameloop({ delta, camera, elements, controls }) {
      ...
      // Y from 3D tile height grid (bilinear)
  ...
  elements: [
    {
      ...
      height: MAP_ROWS,
      tileSize: TILE_SIZE,
      step: TILE_SIZE, // 45° ramps
      // rotations omitted → default ramp bands (lib/utils/tile3d.js)
      position: [0, 0, 0],
      wireframe: true,
      material: {
        type: 'standard',
